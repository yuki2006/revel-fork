...
type templateRuntime struct {
	...
	templateMap map[string]Template
	// Map from template name to its modification time (for hot-reload in dev mode)
	templateModTimes map[string]time.Time
}
...
func (runtimeLoader *templateRuntime) findAndAddTemplate(path, fullSrcDir, basePath string) (fileBytes []byte, err error) {
	...
	// Record file modification time for hot-reload in dev mode
	if info, statErr := os.Stat(path); statErr == nil {
		runtimeLoader.templateModTimes[templateName] = info.ModTime()
	}
...
func (runtimeLoader *templateRuntime) TemplateLang(name, lang string) (tmpl Template, err error) {
	// In dev mode, check if template file has been modified and reload if necessary
	if DevMode {
		if filePath, found := runtimeLoader.TemplatePaths[name]; found {
			if cachedModTime, hasModTime := runtimeLoader.templateModTimes[name]; hasModTime {
				if info, statErr := os.Stat(filePath); statErr == nil {
					if info.ModTime().After(cachedModTime) {
						templateLog.Info("Template modified, refreshing all templates", "name", name, "file", filePath)
						runtimeLoader.loader.Refresh()
						// Reload the runtimeLoader after refresh
						runtimeLoader = runtimeLoader.loader.runtimeLoader.Load().(*templateRuntime)
					}
				}
			}
		}
